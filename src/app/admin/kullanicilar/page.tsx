          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-navy-100/70">
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Ad Soyad</th>
                ...
                <th className="px-6 py-4 text-left text-xs font-semibold text-slate-400 uppercase tracking-wider">Kayıt</th>
                <th className="px-6 py-4 text-right text-xs font-semibold text-slate-400 uppercase tracking-wider">İşlem</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-navy-50">
              {users.map((user) => (
                <tr key={user.id} className="hover:bg-orange-50/60 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-navy-950">{user.name}</span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <select value={user.role} onChange={(e) => updateRole(user.id, e.target.value)}
                        className="text-xs border border-navy-100 rounded-lg px-2.5 py-1.5 bg-white focus:ring-1 focus:ring-orange-400 focus:border-transparent">
          ...
          {users.map((user) => (
            <div key={user.id} className="p-4 space-y-3">
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-slate-400">{new Date(user.createdAt).toLocaleDateString("tr-TR")}</span>
                <div className="flex items-center gap-2">
                  <select value={user.role} onChange={(e) => updateRole(user.id, e.target.value)}
                    className="text-xs border border-navy-100 rounded-lg px-2 py-1 bg-white">
                    <option value="PENDING">Onay Bekliyor</option>
                    <option value="USER">Kullanıcı</option>
                    <option value="ADMIN">Admin</option>
                  </select>
